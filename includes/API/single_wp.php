...
function p23_consult_single_data_to_show_callback($res){
  ...
  // if(!p23_date_validation($res['x|start_date'], $res['end_date'])) return p23_error_response(1, http_response_code());
  ...
  // if (empty($search_ids)) return p23_error_response(4, http_response_code());
  ...
  // if (empty($search_ids)) return p23_error_response(2, http_response_code());
  ...
  if (empty(array_column($search_ids, 'id'))) $resp = $search_ids;
  ...
  $wp_apartments = p23_get_wp_apartments();
  if (empty($wp_apartments)) return p23_error_response(3, http_response_code());
  $wp_apartments = array_map('intval', $wp_apartments);
  ...
  if ($price <= 0) return p23_error_response(10, http_response_code());
  if (empty($smoobu_id)) return p23_error_response(16, http_response_code());
  if (empty($product_id)) return p23_error_response(16, http_response_code());
  if (empty($cleaning_charge)) return p23_error_response(16, http_response_code());
